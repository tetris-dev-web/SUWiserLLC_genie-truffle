        ...
        describe('when the project is still open', async () => {
          ...
          it('activates the project', async () => {
            ...
            assert.equal(
              firstUint,
              1700000,
              'should pass leading project captial required to activate project',
            );
          });
        });
    ...
    describe('when sender is not the project factory', async () => {
      it('reverts', async () => {
        await exceptions.catchRevert(
          activation.activateProject(project.address, 1700000, { from: accounts[1] }),
        );
      });
    });
  });
  ...
  describe('_activateProject', async () => {
    ...
    it('increases pending token activations', async () => {
      ...
      assert.equal(
        firstUint,
        20000000,
        'should increase pending active tokens according to the tokens associated witht the activated project',
      );
    });
