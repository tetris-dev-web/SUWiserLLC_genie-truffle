  ...
  describe("createProject", async () => {
    describe("when the sender is the developer", async () => {
      ...
      it("mints new tokens and extend the crowdsale doomsday", async () => {
        const { firstUint, secondUint } = await stubUtil.callHistory(cS, "mintNewProjectTokensAndExtendDoomsDay");
      ...
      it("increments the totalProjectCount by one", async () => {
        ...
        assert.equal(Number(totalProjectCountT2), Number(totalProjectCountT1) + 1, "should increase totalProjectCount by one");
      })


      it("handles a new project", async () => {
        const { firstAddress } = await stubUtil.callHistory(pFH, "handleNewProject");
        assert.equal(firstAddress, projectAddr, "should perform handle new project functionality by the project address");
      })
    })
    describe("when the sender is not the developer", async () => {
      it("reverts", async () => {
        await exceptions.catchRevert(pF.createProject('information', 5000000, 2000000, 'cashflow', {from: accounts[1]}));
      })
    })
  })
})
...
const setUp = async () => {
  ...
  pFH = await ProjectFactoryHelperStub.new(activation.address, voting.address, projectLeaderTracker.address);
  await initCrowdsale(inactiveToken);
  pF = await ProjectFactroyMock.new(
    pFH.address,
    cS.address,
    accounts[0],
    accounts[2]
