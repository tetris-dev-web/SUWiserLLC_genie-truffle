...
module.exports = {
  catchRevert: async function (promise) {
    await testException(promise, 'revert');
  },
  catchOutOfGas: async function (promise) {
    await testException(promise, 'out of gas');
  },
  catchInvalidJump: async function (promise) {
    await testException(promise, 'invalid JUMP');
  },
  catchInvalidOpcode: async function (promise) {
    await testException(promise, 'invalid opcode');
  },
  catchStackOverflow: async function (promise) {
    await testException(promise, 'stack overflow');
  },
  catchStackUnderflow: async function (promise) {
    await testException(promise, 'stack underflow');
  },
  catchStaticStateChange: async function (promise) {
    await testException(promise, 'static state change');
  },
};
